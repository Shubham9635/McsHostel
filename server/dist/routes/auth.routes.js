...
router.post('/login', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: profile.id, email: profile.email, role: profile.role, name: profile.name }, auth_middleware_1.JWT_SECRET_KEY, { expiresIn: '7d' });
...
router.post('/direct-login', async (req, res) => {
    try {
        ...
        if (!profile) {
            ...
            const randomHash = await bcryptjs_1.default.hash(Math.random().toString(36), 10);
            const { data: newProfile } = await supabase_1.supabase.from('profiles').insert({
...
router.post('/send-otp', async (req, res) => {
    try {
        ...
        const otp = (0, otp_service_1.generateOtpCode)();
        // Store OTP (with rate limiting check)
        const storeResult = await (0, otp_service_1.storeOtp)(normalizedEmail, otp);
...
router.post('/verify-otp', async (req, res) => {
    try {
        ...
        if (!['student@hostel.hub', 'admin@hostel.hub'].includes(normalizedEmail)) {
            ...
            const verification = await (0, otp_service_1.verifyOtp)(normalizedEmail, String(otp));
        ...
        if (!profile) {
            ...
            if (isAdminEmail && profile.role !== 'admin') {
                const { data: updatedAdmin } = await supabase_1.supabase
                    .from('profiles')
                    .update({ role: 'admin' })
                    .eq('id', profile.id)
                    .select()
...
router.post('/google-login', async (req, res) => {
    try {
        ...
        if (['student@hostel.hub', 'admin@hostel.hub'].includes(normalizedEmail)) {
            let { data: demoProfile } = await supabase_1.supabase
                .from('profiles')
                .select('*')
                .eq('email', normalizedEmail)
                .single();
            if (demoProfile) {
                const token = jsonwebtoken_1.default.sign({ id: demoProfile.id, email: demoProfile.email, role: demoProfile.role, name: demoProfile.name }, auth_middleware_1.JWT_SECRET_KEY, { expiresIn: '7d' });
        ...
        if (!profile) {
            ...
            const defaultName = name?.trim() || (isAdminEmail
                ? 'Hostel Management'
                : normalizedEmail
                    .split('@')[0]
                    .replace(/[._]/g, ' ')
...
router.post('/register', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: newProfile.id, email: newProfile.email, role: newProfile.role, name: newProfile.name }, auth_middleware_1.JWT_SECRET_KEY, { expiresIn: '7d' });
...
router.patch('/profile', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        if (hostel !== undefined)
            updates.hostel = hostel || null;
        if (block !== undefined)
            updates.block = block || null;
        if (student_id !== undefined)
            updates.student_id = student_id || null;
...
router.post('/change-password', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        if (!profile)
            return res.status(404).json({ error: 'User not found.' });
        const valid = await bcryptjs_1.default.compare(current_password, profile.password_hash);
        if (!valid)

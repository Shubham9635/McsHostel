...
async function seed() {
    ...
    const { data: insertedStaff, error: staffError } = await supabase_1.supabase
        .from('staff').insert(staff).select();
    if (staffError) {
        console.error('Staff seed error:', staffError.message);
        return;
    }
    console.log(`  ✅ Staff: ${insertedStaff?.length} records`);
    // ---- Users ----
    const demoHash = await bcryptjs_1.default.hash('demo123', 12);
    ...
    const { data: insertedUsers, error: userError } = await supabase_1.supabase
        .from('profiles').insert(users).select();
    if (userError) {
        console.error('Users seed error:', userError.message);
        return;
    }
    ...
    const { data: insertedComplaints, error: complaintError } = await supabase_1.supabase
        .from('complaints').insert(complaints).select();
    if (complaintError) {
        console.error('Complaints seed error:', complaintError.message);
        return;
    }
    ...
    const { error: fbError } = await supabase_1.supabase.from('complaint_feedback').insert(feedback);
    if (fbError)
        console.warn('Feedback seed warning:', fbError.message);
    else
    ...
    const { error: histError } = await supabase_1.supabase.from('complaint_status_history').insert(history);
    if (histError)
        console.warn('History seed warning:', histError.message);
    else
        console.log(`  ✅ Status history: ${history.length} records`);
    // ---- Mess Reviews ----
    ...
    const { error: reviewError } = await supabase_1.supabase.from('mess_reviews').insert(reviews);
    if (reviewError)
        console.warn('Reviews seed warning:', reviewError.message);
    else
        console.log(`  ✅ Mess reviews: ${reviews.length} records`);

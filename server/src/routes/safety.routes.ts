...
import { authenticate, AuthRequest, requireAdmin, requireStudent } from '../middleware/auth.middleware';
...
router.post('/reports', requireStudent, async (req: AuthRequest, res: Response) => {
  try {
    const {
      incident_type,
      severity = 'Normal',
    ...
    try {
      await supabase.from('safety_report_history').insert({
        report_id: report.id,
        old_status: 'None',
    ...
    try {
      await NotificationService.notifyStudent(req.user!.id, {
    ...
    const { reporter_user_id, ...safeReport } = report as any;
...
router.get('/reports/my', requireStudent, async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/reports/:id/status', requireStudent, async (req: AuthRequest, res: Response) => {
  try {
    const { data: report, error } = await supabase
      .from('safety_reports')
    ...
    if (report.reporter_user_id !== req.user!.id) return res.status(403).json({ error: 'Access denied.' });
...
router.get('/admin/reports', requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (hostel && hostel !== 'all') {
      query = query.eq('hostel', hostel as string);
    }
...
router.get('/admin/reports/:id', requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    const { data: report, error } = await supabase
      .from('safety_reports')
      .select('id, tracking_id, incident_type, severity, is_immediate_danger, description, hostel, block, floor, location, incident_date, incident_time, status, priority, internal_notes, created_at, updated_at')
...
router.patch('/admin/reports/:id/status', requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (note !== undefined) updates.internal_notes = note;
    ...
    if (status && status !== current.status) {
      ...
      if (current.reporter_user_id) {
        ...
        NotificationService.notifyStudent(current.reporter_user_id, {
...
router.get('/admin/reports/:id/evidence', requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const signedList = await Promise.all((evidence || []).map(async (ev: any) => {

...
router.get('/stats', async (req: AuthRequest, res: Response) => {
  try {
    ...
    let all = complaints || [];
    if (!isDemo) {
      all = all.filter(c => !isDummyComplaint(c));
    }
    ...
    if (!isDemo) {
      reviews = reviews.filter(r => !isDummyMessReview(r));
    }
    ...
    let staffList = staff || [];
    if (!isDemo) {
      staffList = staffList.filter(s => !isDummyStaff(s));
    }
    ...
    if (isDemo) {
      ...
      const { data: realStudents } = await supabase
        .from('profiles')
        .select('id, email')
        .eq('role', 'student');
      const filtered = (realStudents || []).filter(s => !isDummyStudent(s));
...
router.get('/students', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!isDemo) {
      studentList = studentList.filter(s => !isDummyStudent(s));
    }
    ...
    if (!isDemo) {
      validComplaints = validComplaints.filter(c => !isDummyComplaint(c));
    }
...
router.patch('/complaints/:id/assign', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (error || !updated) return res.status(500).json({ error: 'Failed to assign staff.' });
